<?php
    ...
    if ($stmt = $conn->prepare("SELECT name,code,description,author,added,id FROM codes WHERE id = ? LIMIT 1")) {
    ...
    $query = "SELECT author,content,date FROM comments WHERE codeId='$cId' ORDER BY id DESC";
    
    echo "<h2>Kommentit</h2>";
    
    $stmt->fetch();
    
    if ($stmt = $conn->prepare("SELECT author,content,date FROM comments WHERE codeId = ? ORDER BY id ASC")) {
        $stmt->bind_param("i",$codeId);
        $stmt->execute();
